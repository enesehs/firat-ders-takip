...
function programGuncelle() {
  ...
  if (bugunDersler.length === 0) {
    el.programListesi.innerHTML = '<div class="no-class">Bugün dersin yok, keyfine bak! 🎉</div>';
    heroGuncelle('Tatil Modu', 'Ders Yok', 'Dinlenme Zamanı', 'İyi tatiller!', false, false);
    return;
  }
...
function derseKatil() {
  chrome.storage.local.get(['githubTakipEdildi', 'githubStarEdildi'], (sonuc) => {
    const takipTamam = sonuc.githubTakipEdildi === true;
    const starTamam = sonuc.githubStarEdildi === true;

    if (!takipTamam || !starTamam) {
      takipOverlayGoster(takipTamam, starTamam);
      return;
    }
...
function takipOverlayGoster(takipTamam, starTamam) {
  if (document.getElementById('takip-overlay')) return;

  let baslik, mesaj, butonMetni, hedefUrl;

  if (!takipTamam) {
    baslik = 'Beni Takip Et';
    mesaj = "Bu eklentiyi kullanmaya devam etmek için lütfen önce GitHub'da beni takip et.";
    butonMetni = "GitHub'da Takip Et";
    hedefUrl = GITHUB_PROFIL_URL;
  } else if (!starTamam) {
    baslik = 'Projeyi Yıldızla';
    mesaj = "Harika! Şimdi de bu projeyi yıldızlayarak (Star) destek olabilir misin?";
    butonMetni = "Projeyi Yıldızla";
    hedefUrl = 'https://github.com/enesehs/firat-ders-takip';
  } else {
    return;
  }
  ...
  overlay.innerHTML = `
    <div style="position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.95); z-index:9999; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:20px; text-align:center;">
      ...
      <h2 style="color:white; margin:0 0 10px; font-size:18px;">${baslik}</h2>
      <p style="color:#aaa; font-size:13px; margin:0 0 20px; max-width:280px;">${mesaj}</p>
  ...
  document.getElementById('takipBtn').addEventListener('click', () => {
    chrome.tabs.create({ url: hedefUrl });
  });
}
